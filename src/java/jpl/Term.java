...
public abstract class Term {
	...
	public String blobType() { // overridden in Atom
		return null;
	}
	...
	public String name() { // overridden in Compound, Variable
		throw new JPLException("name() is undefined for " + this.typeName() + " instances");
	};
	...
	public Term[] toTermArray() {
		try {
			int len = this.listLength(); // exception if not LIST
	...
	public void put(term_t term) {
		put(new Hashtable<String, term_t>(), term);
	}
	 ...
	 * @param args
	 *            An array of jpl.Term references.
	...
	/**
	 * This method calls from_term_t on each term in the n consecutive term_ts. A temporary jpl.term_t "holder" (byref) structure must be created in order to extract type information from the Prolog
	 * engine.
	 * 
	 * @param vars_to_Vars
	 *            A Map from Prolog variables to jpl.Variable instances
	 * @param n
	...
	/**
	 * create and return a jpl.Term representation of the given Prolog term
	 * 
	 * @param vars_to_Vars
	 *            A Map from Prolog variables to jpl.Variable instances
	 * @param term
	...
	protected static Term getTerm(Map<term_t, Variable> vars_to_Vars, term_t term) {
		...
		case Prolog.LIST_NIL:
			return JPL.LIST_NIL;
		default:
			throw new JPLException("Term.from_term_t: unknown term type=" + Prolog.term_type(term)); // should never happen...
		}
	}

	protected static Term getTerm(term_t term) {
		return getTerm(new Hashtable<term_t, Variable>(), term);
	}
	...
	/**
	 * This method computes a substitution from a Term. The bindings Hashtable stores Terms, keyed by names of Variables. Thus, a substitution is as it is in mathematical logic, a sequence of the form
	 ...
	 * <p>
	 * 
	 * A second Hashtable, vars, is required; this table holds the Variables that occur (thus far) in the unified term. The Variable instances in this table are guaranteed to be unique and are keyed
	 * on Strings which are Prolog internal representations of the variables.
	 * 
	 * @param bindings
	 *            table holding Term substitutions, keyed on names of Variables.
	 * @param vars
	 *            A Hashtable holding the Variables that occur thus far in the term; keyed by internal (Prolog) string rep.
	 */
