...
public class Compound extends Term {
	...
	/**
	 * Creates a Compound with name but no args (i.e. an Atom). This constructor is protected (from illegal public use) and is used only by Atom, which inherits it.
	 * 
	 * @param name
	 *            the name of this Compound
	 * @param args
	...
	protected Compound(String name) {
		if (name == null) {
			throw new JPLException("jpl." + this.typeName() + ": cannot construct with null name");
		} else {
			this.name = name;
			this.args = new Term[] {};
		}
	...
	/**
	 * Returns a prefix functional representation of a Compound of the form name(arg1,...), where 'name' is quoted iff necessary (to be valid Prolog soutce text) and each argument is represented
	 * according to its toString() method.
	 * 
	...
	protected String quotedName() {
		...
		// }))).oneSolution().get(
		// "S")).name;
		// return ((Atom) (new Query(new Compound("sformat", new Term[] { new Variable("S"), new Atom("~q"), new Compound(JPL.LIST_PAIR, new Term[] { new Atom(this.name), JPL.LIST_NIL }) })))
		// .oneSolution().get("S")).name;
		return (isSimpleName(name) ? name : "'" + name + "'");
	}

	private boolean isSimpleName(String s) {
		if (s == null) {
