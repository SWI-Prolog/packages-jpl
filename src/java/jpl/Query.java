 ...
 * (otherwise false), and if the call or redo did succeed, the nextElement() method returns a Hashtable representing variable bindings; the elements in the Hashtable are Terms, indexed by the (String)
 * names of the Variables with which they are associated. For example, if <i>p(a)</i> and <i>p(b)</i> are facts in the Prolog database, then the following is equivalent to printing all the solutions
 * to the Prolog query <i>p(X)</i>:
 * 
 * <pre>
 ...
 * @version $Revision$
 */
public class Query implements Enumeration<Object> {
	private static Map<Long, Query> m = new Hashtable<Long, Query>(); // maps (engine_t) engine handle to (Query) topmost query
	/**
	 ...
	 * @return the name of this Query's goal
	 */
	 ...
	 * @return the arguments of this Query's goal
	 */
	 ...
	 * substitutions in the form of Hashtables. To iterate through all the solutions to a Query, for example, one might write
	 * 
	 * <pre>
	...
	public synchronized final void open() {
		...
		Map<String, term_t> varnames_to_vars = new Hashtable<String, term_t>();
	...
	private final boolean get1() { // try to get the next solution; if none, close the query;
		if (Prolog.next_solution(qid)) {
			...
			if (exception_term_t.value != 0L) {
				Term exception_term = Term.getTerm(new Hashtable<term_t, Variable>(), exception_term_t);
				close();
	 ...
	 */
	public synchronized final Hashtable<String, Term> getSolution() {
	...
	public synchronized final Hashtable<String, Term> getSubstWithNameVars() {
		// oughta check: thread has query's engine
		if (!open) {
			throw new JPLException("Query is not open");
		} else if (get1()) {
	 ...
	 */
	public synchronized final Hashtable<String, Term> nextSolution() {
		return get2();
	}

	private final Hashtable<String, Term> get2() {
		if (!open) {
			throw new JPLException("Query is not open");
		} else {
			Hashtable<String, Term> substitution = new Hashtable<String, Term>();
			// NB I reckon computeSubstitutions needn't be in Term (but where else?)
			Term.getSubsts(substitution, new Hashtable<term_t, Variable>(), goal_.args);
			return substitution;
		}
	}

	// assumes that Query's last arg is a Variable which will be bound to a [Name=Var,..] dict
	private final Hashtable<String, Term> get2WithNameVars() {
		if (!open) {
			throw new JPLException("Query is not open");
		} else {
			...
			Map<String, Term> varnames_to_Terms1 = new Hashtable<String, Term>();
			Map<term_t, Variable> vars_to_Vars1 = new Hashtable<term_t, Variable>();
			args[args.length - 1].getSubst(varnames_to_Terms1, vars_to_Vars1);
			Hashtable<String, Term> varnames_to_Terms2 = new Hashtable<String, Term>();
	...
	/**
	 * This method can be used to close an open query before its solutions are exhausted. It is called automatically when solutions are exhausted, i.e. when hasMoreSolutions() fails. Calling close()
	 * on an already closed Query is harmless (has no effect).
	 * <p>
	 * 
	...
	/**
	 * calls the Query's goal to exhaustion and returns an array of zero or more Hashtables of zero or more variablename-to-term bindings (each Hashtable represents a solution, in the order in which
	 * they were found).
	 * 
	 * @return an array of zero or more Hashtables of zero or more variablename-to-term bindings (each Hashtable represents a solution, in the order in which they were found) <b>NB</b> in JPL 1.0.1,
	 *         this method (inconsistently) returned null when a Query had no solutions; in JPL 2.x onwards it returns an empty array (thus the length of the array is, in every case, the quantity of
	 *         solutions).
	 *         <p>
	 ...
	 *         <p>
	 */
	public synchronized final Hashtable<String, Term>[] allSolutions() {
		if (open) {
			throw new JPLException("Query is already open");
		} else {
			// get a vector of solutions:
			Vector<Hashtable<String, Term>> v = new Vector<Hashtable<String, Term>>();
			while (hasMoreSolutions()) {
				v.addElement(nextSolution());
			}
			// turn the vector into an array:
			@SuppressWarnings("unchecked")
			Hashtable<String, Term> solutions[] = (Hashtable<String, Term>[]) new Hashtable[v.size()]; // 0 solutions -> Hashtable[0]
	...
	/**
	 * This static method creates a Query whose goal is the given Term, calls it to exhaustion, and returns an array of zero or more Hashtables of zero or more variablename-to-term bindings (each
	 * Hashtable represents a solution, in the order in which they were found). Throws JPLException if goal is neither a jpl.Atom nor a jpl.Compound.
	 * 
	 ...
	 *            the goal of this Query
	 */
	public static final Hashtable<String, Term>[] allSolutions(Term goal) {
		return (new Query(goal)).allSolutions();
	}

	/**
	 * This static method creates a Query from the given Prolog source text fragment, calls it to exhaustion, and returns an array of zero or more Hashtables of zero or more variablename-to-term
	 * bindings (each Hashtable represents a solution, in the order in which they were found). Throws PrologException containing error(syntax_error(_),_) if text is invalid.
	 * 
	 * @return an array of zero or more Hashtables of zero or more variablename-to-term bindings (each Hashtable represents a solution, in the order in which they were found)
	 * 
	 * @param text
	 ...
	 */
	public static final Hashtable<String, Term>[] allSolutions(String text) {
	 ...
	 * by its respective param, calls the resulting goal to exhaustion, and returns an array of zero or more Hashtables of zero or more variablename-to-term bindings (each Hashtable represents a
	 ...
	 */
	public static final Hashtable<String, Term>[] allSolutions(String text, Term[] params) {
		return (new Query(text, params)).allSolutions();
	}

	/**
	 * calls the Query's goal to exhaustion or until N solutions are found, whichever is sooner, and returns an array containing (as possibly empty Hashtables of variablename-to-term bindings) every
	 * found solution (in the order in which they were found).
	 * 
	 * @return an array of Hashtables (possibly none), each of which is a solution (in the order in which they were found) of the Query; at most 'n' solutions will be found and returned. <b>NB</b> in
	 *         JPL 1.0.1, this method (inconsistently) returned null when a Query had no solutions; in JPL 2.x onwards it returns an empty array (thus the length of the array is, in every case, the
	 ...
	 *         <p>
	 */
	public synchronized final Hashtable<String, Term>[] nSolutions(long n) {
		if (open) {
			throw new JPLException("Query is already open");
		} else {
			// get a vector of solutions:
	 ...
	 * Hashtables of variablename-to-term bindings) every found solution (in the order in which they were found). Throws JPLException if goal is neither a jpl.Atom nor a jpl.Compound.
	 * 
	 * @param goal
	 *            the goal of this Query
	 */
	public static final Hashtable<String, Term>[] nSolutions(Term goal, long n) {
	 ...
	 * possibly empty Hashtables of variablename-to-term bindings) every found solution (in the order in which they were found). Throws PrologException containing error(syntax_error(_),_) if text is
	 * invalid.
	 * 
	 * @param text
	 *            a Prolog source text fragment denoting a goal
	 */
	public static final Hashtable<String, Term>[] nSolutions(String text, long n) {
	 ...
	 * its respective param, calls the resulting goal to exhaustion or until N solutions are found, whichever is sooner, and returns an array containing (as possibly empty Hashtables of
	 * variablename-to-term bindings) every found solution (in the order in which they were found).
	 * 
	 ...
	 *            terms to be substituted for the respective questionmarks in the query text
	 */
	public static final Hashtable<String, Term>[] nSolutions(String text, Term[] params, long n) {
	 ...
	 * 
	 * @return the first solution, if the query has one, as a (possibly empty) Hashtable. If the return value is null, this means that the Query has no solutions.
	 *         <p>
	 */
	public synchronized final Hashtable<String, Term> oneSolution() {
		if (open) {
			throw new JPLException("Query is already open");
		} else {
	...
	/**
	 * This static method creates a Query (whose goal is the specified Term) and calls it at most once, returning the first solution, if there is one, as a (possibly empty) Hashtable, else null. The
	 * goal can be a jpl.Atom or a jpl.Compound, but cannot be an instance of jpl.Float, jpl.Integer or jpl.Variable.
	 * 
	 * @param goal
	 *            the goal of this Query
	 */
	public static final Hashtable<String, Term> oneSolution(Term goal) {
		return (new Query(goal)).oneSolution();
	}

	/**
	 * This static method creates a Query from the given Prolog source text fragment, and calls it at most once, returning the first solution, if there is one, as a (possibly empty) Hashtable, else
	 * null. Throws PrologException containing error(syntax_error(_),_) if text is invalid.
	 * 
	 * @param text
	 *            a Prolog source text fragment denoting a goal
	 */
	public static final Hashtable<String, Term> oneSolution(String text) {
	 ...
	 * provide the goal of this query: the resulting goal is then called (at most once) and the first solution, if there is one, is returned as a (possibly empty) Hashtable, else null.
	 * 
	 * Otherwise, if text denotes an atom, this static method creates a Query where text is the name of the goal and params are the args; the resulting goal is then called as above. This latter mode
	 * is redundant, deprecated (informally), and retained only for backward compatibility.
	 * 
	 ...
	 *            terms to be substituted for the respective questionmarks in the query text
	 */
	public static final Hashtable<String, Term> oneSolution(String text, Term[] params) {
	...
	public final int abort() {
		if (open) {
			...
			/*
			 * int rc0a = Prolog.pool_engine_id(this.engine); System.out.println("q.abort(): this.engine has id=" + rc0a);
			 * 
			 * engine_t e = Prolog.current_engine(); System.out.println("q.abort(): " + (e == null ? "no current engine" : "current engine id=" + Prolog.pool_engine_id(e)));
			 * 
			 * int rc0b = Prolog.release_pool_engine(); System.err.println("q.abort(): release_pool_engine() returns " + rc0b);
			 * 
			 * engine_t e2 = Prolog.current_engine(); System.out.println("q.abort(): " + (e == null ? "no current engine" : "current engine id=" + Prolog.pool_engine_id(e2)));
			 * 
			 * int rc1 = Prolog.attach_engine(this.engine); System.out.println("q.abort(): attach_engine() returns " + rc1);
			 * 
			 * engine_t e3 = Prolog.current_engine(); System.out.println("q.abort(): " + (e == null ? "no current engine" : "current engine id=" + Prolog.pool_engine_id(e3)));
			 * 
			 * int rc2 = Prolog.action_abort(); System.out.println("q.abort(): action_abort() returns " + rc2);
			 * 
			 * int rc3 = Prolog.release_pool_engine(); System.out.println("q.abort(): release_pool_engine() returns " + rc3);
			 * 
			 * int rc4 = Prolog.attach_engine(e); System.out.println("q.abort(): attach_engine() returns " + rc4);
			 */
			return 0;
		} else {
